...
Use the `/files` endpoints to move files between your local machine and AxieStudio.
...
- `/v2` files are organized by `user_id` instead of `flow_id`.
  ...
  You can upload a file to AxieStudio one time, and use it with multiple flows.
- `/v2` files are tracked in the AxieStudio database.
...
```bash
curl -X POST \
  "$AxieStudio_URL/api/v1/files/upload/$FLOW_ID" \
...
Send image files to AxieStudio to use them in flows.

The default file limit is 100 MB.
To change this limit, set the `AxieStudio_MAX_FILE_SIZE_UPLOAD` [environment variable](/environment-variables).

1. Attach the image to a `POST /v1/files/upload/$FLOW_ID` request with `--form` (`-F`) and the file path:

    ```bash
    curl -X POST "$AxieStudio_URL/api/v1/files/upload/$FLOW_ID" \
      -H "Content-Type: multipart/form-data" \
      -H "x-api-key: $AxieStudio_API_KEY" \
      -F "file=@PATH/TO/FILE.png"
    ```

    A successful request returns the `file_path` for the image in the AxieStudio file management system in the format `FLOW_ID/TIMESTAMP_FILENAME.TYPE`.
    For example:
...
2. Use the returned `file_path` to send the image file to other components that can accept file input. Where you specify the file path depends on the component type.
    ...
    ```bash
    curl -X POST \
        "$AxieStudio_URL/api/v1/run/a430cc57-06bb-4c11-be39-d3d4de68d2c4?stream=false" \
...
```bash
curl -X GET \
  "$AxieStudio_URL/api/v1/files/list/$FLOW_ID" \
  -H "accept: application/json" \
  -H "x-api-key: $AxieStudio_API_KEY"
```
...
```bash
curl -X GET \
  "$AxieStudio_URL/api/v1/files/download/$FLOW_ID/2024-12-30_15-19-43_your_file.txt" \
  -H "accept: application/json" \
  -H "x-api-key: $AxieStudio_API_KEY" \
...
```bash
curl -X DELETE \
  "$AxieStudio_URL/api/v1/files/delete/$FLOW_ID/2024-12-30_15-19-43_your_file.txt" \
  -H "accept: application/json" \
  -H "x-api-key: $AxieStudio_API_KEY"
```
...
To create a AxieStudio API key and export it as an environment variable, see [Get started with the AxieStudio API](/api-reference-api-examples).
...
1. To retrieve your current `user_id`, call the `/whoami` endpoint:
    ...
    ```bash
    curl -X GET \
      "$AxieStudio_URL/api/v1/users/whoami" \
      -H "accept: application/json" \
      -H "x-api-key: $AxieStudio_API_KEY"
    ```
    ...
    ```
    {"id":"07e5b864-e367-4f52-b647-a48035ae7e5e","username":"AxieStudio","profile_image":null,"store_api_key":null,"is_active":true,"is_superuser":true,"create_at":"2025-05-08T17:59:07.855965","updated_at":"2025-05-28T19:00:42.556460","last_login_at":"2025-05-28T19:00:42.554338","optins":{"github_starred":false,"dialog_dismissed":true,"discord_clicked":false,"mcp_dialog_dismissed":true}}
    ```
...
This endpoint uploads files to your AxieStudio server's file management system.
To use an uploaded file in a flow, send the file path to a flow with a [**File** component](/components-data#file).

The default file limit is 100 MB. To configure this value, change the `AxieStudio_MAX_FILE_SIZE_UPLOAD` [environment variable](/environment-variables).
...
This component loads files into flows from your local machine or AxieStudio file management.

3. Run the flow, passing the `path` to the `File` component in the `tweaks` object:

    ```text
    curl --request POST \
      --url "$AxieStudio_URL/api/v1/run/$FLOW_ID" \
...
```bash
curl -X GET \
  "$AxieStudio_URL/api/v2/files/c7b22c4c-d5e0-4ec9-af97-5d85b7657a34" \
  -H "accept: application/json" \
  -H "x-api-key: $AxieStudio_API_KEY" \
  --output downloaded_file.txt
```
...
```bash
curl -X PUT \
  "$AxieStudio_URL/api/v2/files/$FILE_ID?name=new_file_name" \
  -H "accept: application/json" \
  -H "x-api-key: $AxieStudio_API_KEY"
```
...
```bash
curl -X DELETE \
  "$AxieStudio_URL/api/v2/files/$FILE_ID" \
  -H "accept: application/json" \
  -H "x-api-key: $AxieStudio_API_KEY"
```
