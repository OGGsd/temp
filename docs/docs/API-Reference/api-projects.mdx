...
Use the `/projects` endpoint to create, read, update, and delete [AxieStudio projects](/concepts-flows#projects).

## Read projects

Get a list of AxieStudio projects, including project IDs, names, and descriptions.

```bash
curl -X GET \
  "$AxieStudio_URL/api/v1/projects/" \
...
```bash
curl -X GET \
  "$AxieStudio_URL/api/v1/projects/$PROJECT_ID" \
  -H "accept: application/json" \
  -H "x-api-key: $AxieStudio_API_KEY"
```
...
```bash
curl -X PATCH \
  "$AxieStudio_URL/api/v1/projects/b408ddb9-6266-4431-9be8-e04a62758331" \
  -H "accept: application/json" \
  -H "x-api-key: $AxieStudio_API_KEY" \
  -d '{
  "name": "string",
...
```bash
curl -X GET \
  "$AxieStudio_URL/api/v1/projects/download/$PROJECT_ID" \
  -H "accept: application/json" \
  -H "x-api-key: $AxieStudio_API_KEY" \
  --output AxieStudio-project.zip
```

## Import a project

Import a project and its flows by uploading a AxieStudio project zip file:

```bash
curl -X POST \
  "$AxieStudio_URL/api/v1/projects/upload/" \
  -H "accept: application/json" \
  -H "Content-Type: multipart/form-data" \
  -H "x-api-key: $AxieStudio_API_KEY" \
  -F "file=@20241230_135006_AxieStudio_flows.zip;type=application/zip"
```