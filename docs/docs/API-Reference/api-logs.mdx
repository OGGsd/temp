...
Retrieve logs for your AxieStudio flow.

## Enable log retrieval

The `/logs` endpoint requires log retrieval to be enabled in your AxieStudio instance.
...
1. To enable log retrieval, include these values in your `.env` file:
    ...
    Log retrieval requires that `AxieStudio_LOG_RETRIEVER_BUFFER_SIZE` is greater than 0. The default value is `10000`.

2. Start AxieStudio with the updated `.env`:
...
```bash
curl -X GET \
  "$AxieStudio_URL/logs?lines_before=0&lines_after=0&timestamp=0" \
...
```text
{
  ...
  "1736354775699": "2025-01-08T11:46:15.699661-0500 DEBUG File 046-rocket.svg retrieved successfully from flow /Users/USER/Library/Caches/AxieStudio/profile_pictures/Space.\n"
}
```
