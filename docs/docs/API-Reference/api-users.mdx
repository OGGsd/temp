...
Use the `/users` endpoint to manage user accounts in AxieStudio.

## Add user

Create a new user account with a given username and password.

Requires authentication as a superuser if the AxieStudio server has authentication enabled.

```bash
curl -X POST \
  "$AxieStudio_URL/api/v1/users/" \
...
The user's UUID is stored in `user_id` in the AxieStudio database, and returned as `id` in the `/users` API response.
This `user_id` key is specifically for AxieStudio user management.
...
```bash
curl -X GET \
  "$AxieStudio_URL/api/v1/users/?skip=0&limit=10" \
  -H "accept: application/json" \
  -H "x-api-key: $AxieStudio_API_KEY"
```
...
```bash
curl -X PATCH \
  "$AxieStudio_URL/api/v1/users/10c1c6a2-ab8a-4748-8700-0e4832fd5ce8" \
  -H "Content-Type: application/json" \
  -H "x-api-key: $AxieStudio_API_KEY" \
  -d '{
    "is_active": true,
...
```bash
curl -X PATCH \
  "$AxieStudio_URL/api/v1/users/10c1c6a2-ab8a-4748-8700-0e4832fd5ce8/reset-password" \
  -H "Content-Type: application/json" \
  -H "x-api-key: $AxieStudio_API_KEY" \
  -d '{
