...
:::important
The `/build` endpoints are used by AxieStudio's frontend visual editor code.
These endpoints are part of the internal AxieStudio codebase.

Don't use these endpoints to run flows in applications that use your AxieStudio flows.
To run flows in your apps, see [Flow trigger endpoints](/api-flows-run).
:::

The `/build` endpoints support AxieStudio's frontend code for building flows in the AxieStudio visual editor.
You can use these endpoints to build vertices and flows, as well as execute flows with streaming event responses.
You might need to use or understand these endpoints when contributing to the AxieStudio codebase.
...
1. Send a POST request to the `/build/$FLOW_ID/flow` endpoint:
    ...
    ```shell
    curl -X POST \
      "$AxieStudio_URL/api/v1/build/$FLOW_ID/flow" \
...
2. After receiving a job ID from the build endpoint, use the `/build/$JOB_ID/events` endpoint to stream the execution results:
    ...
    ```shell
    curl -X GET \
      "$AxieStudio_URL/api/v1/build/123e4567-e89b-12d3-a456-426614174000/events" \
      -H "accept: application/json" \
      -H "x-api-key: $AxieStudio_API_KEY"
    ```
...
```shell
curl -X GET \
  "$AxieStudio_URL/api/v1/build/123e4567-e89b-12d3-a456-426614174000/events?stream=false" \
  -H "accept: application/json" \
  -H "x-api-key: $AxieStudio_API_KEY"
```
...
| start_component_id | string | Optional. ID of the component where the execution should start. Component `id` values can be found in [AxieStudio JSON files](/concepts-flows-import#AxieStudio-json-file-contents) |
| stop_component_id | string | Optional. ID of the component where the execution should stop. Component `id` values can be found in [AxieStudio JSON files](/concepts-flows-import#AxieStudio-json-file-contents).|
...
The `/build` endpoint also accepts inputs for `data` directly, instead of using the values stored in the AxieStudio database.
