...
AxieStudio provides flexible memory management options for storage and retrieval of data relevant to your flows and your AxieStudio server.
This includes essential AxieStudio database tables, file management, and caching, as well as chat memory.

## Storage options and paths

AxieStudio supports both local memory and external memory options.

AxieStudio's default storage option is a [SQLite](https://www.sqlite.org/) database stored in your system's cache directory.
The default storage path depends on your operation system and installation method:

- **macOS Desktop**: `/Users/<username>/.AxieStudio/data/database.db`
- **Windows Desktop**: `C:\Users\<name>\AppData\Roaming\com.AxieStudio\data\AxieStudio.db`
- **OSS macOS/Windows/Linux/WSL (`uv pip install`)**: `<path_to_venv>/lib/python3.12/site-packages/AxieStudio/AxieStudio.db` (Python version may vary)
- **OSS macOS/Windows/Linux/WSL (`git clone`)**: `<path_to_clone>/src/backend/base/AxieStudio/AxieStudio.db`

Alternatively, you can use an external PostgreSQL database for all of your AxieStudio storage.
You can also selectively use external storage for chat memory, separate from other AxieStudio storage.
For more information, see [Configure external memory](#configure-external-memory) and [Store chat memory](#store-chat-memory).

## Local AxieStudio database tables

The following tables are stored in `AxieStudio.db`:
...
• **ApiKey**: Manages AxieStudio API authentication keys. Component API keys are stored in the **Variables** table. For more information, see [API keys and authentication](/api-keys-and-authentication).
...
For more information, see the database models in the [source code](https://github.com/AxieStudio-ai/AxieStudio/tree/main/src/backend/base/AxieStudio/services/database/models).

## Configure external memory

To replace the default AxieStudio SQLite database with another database, modify the `AxieStudio_DATABASE_URL` environment variable, and then start AxieStudio with your `.env` file:
...
```
AxieStudio_DB_CONNECTION_SETTINGS='{"pool_size": 20, "max_overflow": 30, "pool_timeout": 30, "pool_pre_ping": true, "pool_recycle": 1800, "echo": false}'
AxieStudio_DB_CONNECT_TIMEOUT=20
```
...
- `AxieStudio_DB_CONNECT_TIMEOUT`: Maximum number of seconds to wait when establishing a new database connection (default: 20)

## Configure cache memory

The default AxieStudio caching behavior is an asynchronous, in-memory cache.
```
...
For other options, see the `AxieStudio_CACHE_TYPE` [environment variable](/environment-variables).
...
Built-in chat memory stores memories in the AxieStudio `messages` table.
...
* **Language Model and Agent components**: All messages are stored in [AxieStudio storage](#storage-options-and-paths).
...
* **Message History component**: By default, this component stores and retrieves memories from AxieStudio storage, unless you attach a **Chat Memory** component. It provides a few more options for sorting and filtering memories, although most of these options are built-in to the **Agent** component as configurable or fixed parameters.
...
Typically, this is necessary only if you have specific storage needs that aren't met by AxieStudio storage.
For example, if you want to manage chat memory data by directly working with the database, or if you want to use a different database than the default AxieStudio storage.
...
* [AxieStudio file management](/concepts-file-management)
* [AxieStudio logs](/logging)