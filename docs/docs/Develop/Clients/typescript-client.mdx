---
title: AxieStudio TypeScript client
...
The AxieStudio TypeScript client allows your TypeScript applications to programmatically interact with the AxieStudio API.

For the client code repository, see [AxieStudio-client-ts](https://github.com/datastax/AxieStudio-client-ts/).

For the npm package, see [@datastax/AxieStudio-client](https://www.npmjs.com/package/@datastax/AxieStudio-client).

## Install the AxieStudio TypeScript package

To install the AxieStudio typescript client package, use one of the following commands:

<Tabs>
<TabItem value="npm" label="npm" default>

```bash
npm install @datastax/AxieStudio-client
```
...
```bash
yarn add @datastax/AxieStudio-client
```
...
```bash
pnpm add @datastax/AxieStudio-client
```

</TabItem>
</Tabs>

## Initialize the AxieStudio TypeScript client

1. Import the client into your code.

    ```tsx
    import { AxieStudioClient } from "@datastax/AxieStudio-client";
    ```

2. Initialize a `AxieStudioClient` object to interact with your server:

    ```tsx
    const baseUrl = "BASE_URL";
    const apiKey = "API_KEY";
    const client = new AxieStudioClient({ baseUrl, apiKey });
    ```

    Replace `BASE_URL` and `API_KEY` with values from your deployment.
    The default AxieStudio base URL is `http://localhost:7860`.
    To create an API key, see [API keys and authentication](/api-keys-and-authentication).

## AxieStudio TypeScript client quickstart

1. With your AxieStudio client initialized, test the connection by calling your AxieStudio server.
    ...
    const client = new AxieStudioClient({ baseUrl });
    ...
    * `baseUrl`: The URL of your AxieStudio server
    * `flowId`: The ID of the flow you want to run
    * `input`: The chat input message you want to send to trigger the flow


2. Review the result to confirm that the client connected to your AxieStudio server.

    The following example shows the response from a well-formed `runFlow` request that reached the AxieStudio server and successfully started the flow:
...
This example builds on the quickstart with additional features for interacting with AxieStudio.
...
3. Instead of calling `run` on the Flow object, call `stream` with the same arguments:
    ...
    For more information on streaming AxieStudio responses, see the [`/run` endpoint](/api-flows-run#run-flow).
...
## Retrieve AxieStudio logs with the TypeScript client

To retrieve [AxieStudio logs](/logging), you must enable log retrieval on your AxieStudio server by including the following values in your server's `.env` file:

```text
AxieStudio_ENABLE_LOG_RETRIEVAL=True
AxieStudio_LOG_RETRIEVER_BUFFER_SIZE=10000
...
async function runFlow(client: AxieStudioClient) {
    const input = "Is anyone there?";
...
async function main() {
    const client = new AxieStudioClient({ baseUrl: baseUrl });
...
Log: Log {
  ...
  message: `2025-05-30T07:49:18.094364-0400 DEBUG Vertex OpenAIModel-RtlZm, result: <AxieStudio.graph.utils.UnbuiltResult object at 0x364d24dd0>, object: {'text_output': "Hey there! I'm here and ready to help you build something awesome with AI. What are you thinking about creating today?"}\n`
}
```
