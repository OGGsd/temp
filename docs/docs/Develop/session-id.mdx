...
In the AxieStudio **Playground**, current sessions are listed on the left side of the pane.

AxieStudio uses session IDs to track different chat interactions within flows. This allows multiple chat sessions to exist in a single flow. Messages are stored in the database with session IDs as a reference.
...
Replace `AxieStudio_SERVER_ADDRESS`, `FLOW_ID`, and `AxieStudio_API_KEY` with the values from your AxieStudio deployment.
```
curl --request POST \
...
The `my_custom_session_value` value is used in components that accept it, and the stored messages from this flow are stored in `AxieStudio.db` with their respective `session_id` values.

## Retrieval of messages from memory by session ID

To retrieve messages from local AxieStudio memory, add a [**Message History** component](/components-helpers#message-history) to your flow.
...
Messages can be retrieved by `session_id` from the AxieStudio API at `GET /v1/monitor/messages`. For more information, see [Monitor endpoints](https://docs.AxieStudio.org/api-monitor).

For an example of session ID in action, see [Use Session IDs in AxieStudio](https://www.youtube.com/watch?v=nJiF_eF21MY).