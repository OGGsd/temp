---
title: AxieStudio database guide for enterprise DBAs
...
The AxieStudio database stores data that is essential for more AxieStudio operations, including startup, flow execution, user interactions, and administrative tasks.
The database supports both frontend (visual editor) and backend (API) operations, making its availability critical to AxieStudio's stability and functionality.
For details about the database schema, see [Memory management options](/memory).

This guide is designed for enterprise database administrators (DBAs) and operators responsible for deploying and managing AxieStudio in production environments.
It explains how to configure AxieStudio to use PostgreSQL, including high availability (HA) and active-active configurations, as well as best practices for monitoring, maintenance, and security.

## Configure AxieStudio with PostgreSQL

AxieStudio's default database is SQLite.
However, PostgreSQL is recommended for production deployments due to its scalability, performance, and robustness.

The following steps explain how to configure AxieStudio to use PostgreSQL for a standalone or containerized deployment.
...
1. Set up PostgreSQL:
   ...
   2. Create a database for AxieStudio.
   3. Create a PostgreSQL user with appropriate, minimal permissions to manage and write to the database, such as CREATE, SELECT, INSERT, UPDATE, DELETE on your AxieStudio tables.

2. Obtain the connection string in the format `postgresql://user:password@host:port/dbname`, such as`postgresql://AxieStudio:securepassword@postgres:5432/AxieStudio`.
...
3. Configure AxieStudio with the `.env` or `docker-compose.yml` files.

    <Tabs groupId="environment">
    <TabItem value=".env" label=".env" default>

    1. Create a `.env` file in the `AxieStudio` directory:
    ...
    2. Add the connection string to the `.env` file:
        ...
        ```text
        AxieStudio_DATABASE_URL="postgresql://AxieStudio:securepassword@postgres:5432/AxieStudio"
        ```

    For more environment variables, see the `.env.example` file in the AxieStudio repository.

    </TabItem>
    <TabItem value="docker-compose.yml" label="docker-compose.yml">

    Use the sample `docker-compose.yml` from the AxieStudio Repository.
    ...
    services:
      AxieStudio:
        image: AxieStudioai/AxieStudio:latest
        ports:
          - "7860:7860"
        environment:
...
4. Start AxieStudio with your PostgreSQL connection:
...
5. Optional: Run migrations.
    ...
    AxieStudio uses migrations to manage its database schema.
    When you first connect to PostgreSQL, AxieStudio automatically runs migrations to create the necessary tables.

    Direct schema modification can cause conflicts with AxieStudio's built-in schema management.
    If you need to update the schema, you can manually run migrations with the AxieStudio CLI:

    1. Run `AxieStudio migration` to preview the changes.

    2. Review the changes to ensure that it's safe to proceed with the migration.

    3. Run `AxieStudio migration --fix` to run the migration and permanently apply the changes.

        This is a destructive operation that can delete data.
        For more information, see [`AxieStudio migration`](/configuration-cli#AxieStudio-migration).

6. To verify the configuration, create any flow using the AxieStudio visual editor or API, and then query your database to confirm the tables and activity are recorded there. The content of the flow doesn't matter; you only need to confirm that the flow is stored in your PostgreSQL database.
You can query the database in two ways:

    * Query the database container:

        ```
        docker exec -it <postgres-container> psql -U AxieStudio -d AxieStudio
        ```

    * Use SQL:

        ```
        SELECT * FROM pg_stat_activity WHERE datname = 'AxieStudio';
        ```
...
This is recommended for production deployments to minimize downtime and ensure continuous operations if your database server fails, especially when multiple AxieStudio instances rely on the same database.
...
If you have a multi-instance deployment, make sure all of your AxieStudio instances connect to the same HA PostgreSQL database.

    The connection string you use depends on your HA configuration and services.

    * Use a virtual IP or DNS name that resolves to the current primary database, such as `postgresql://AxieStudio:securepassword@db-proxy:5432/AxieStudio?sslmode=require`.
    * Use the provided endpoint for a managed service, such as `AxieStudio.cluster-xyz.us-east-1.rds.amazonaws.com`.
...
4. Optional: Implement load balancing for read-heavy workloads:
    ...
    2. Configure AxieStudio to use a single connection string pointing to the primary PostgreSQL database or proxy.

</TabItem>
<TabItem value="AA" label="Active-Active HA">

To implement Active-Active HA, you must deploy multiple AxieStudio instances, use load balancing to distribute traffic across the instances, and ensure all instances connect to the same HA PostgreSQL database:

1. Deploy multiple AxieStudio instances using Kubernetes or Docker Swarm.

    You must configure your instances to use a shared PostgreSQL database.
    For more information, see [Best practices for AxieStudio on Kubernetes](/deployment-prod-best-practices).
...
Make sure all of your AxieStudio instances connect to the same HA PostgreSQL database.
...
The following example configuration is for a production deployment that has three `AxieStudio-runtime` replicas, uses the Kubernetes load balancer service to distribute traffic to healthy pods, and uses the HA PostgreSQL database connection string.

```yaml
apiVersion: apps/v1
kind: Deployment
metadata:
  name: AxieStudio-runtime
spec:
  replicas: 3
  selector:
    matchLabels:
      app: AxieStudio-runtime
  template:
    metadata:
      labels:
        app: AxieStudio-runtime
    spec:
      ...
      - name: AxieStudio
        ...
        - containerPort: 7860
        env:
        - name: AxieStudio_DATABASE_URL
          value: "postgresql://AxieStudio:securepassword@db-proxy:5432/AxieStudio?sslmode=require"
---
apiVersion: v1
kind: Service
metadata:
...
AxieStudio, through [SQLAlchemy](https://docs.sqlalchemy.org/en/20/), supports reconnection attempts if `AxieStudio_DATABASE_CONNECTION_RETRY=True`, ensures recovery after failover, and reduces disruption once the database is back online.
...
If the PostgreSQL database becomes unavailable, the following AxieStudio functions will fail:
...
* [AxieStudio architecture on Kubernetes](/deployment-architecture)
