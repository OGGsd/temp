...
This tutorial shows you how to connect a JavaScript application to a [AxieStudio agent](/agents).
...
In this example, the application sends a customer's email address to the AxieStudio agent. The agent compares the customer's previous orders within the **Directory** component, searches the web for used versions of those items, and returns three results.
...
If not, modify the `const email = "isabella.rodriguez@example.com"` to search for a value in your dataset.
    ...
    const AxieStudio_SERVER_ADDRESS = 'AxieStudio_SERVER_ADDRESS';
    const FLOW_ID = 'FLOW_ID';
    const AxieStudio_API_KEY = 'AxieStudio_API_KEY';
    ...
    async function runAgentFlow(): Promise<void> {
        try {
            ...
            const client = new AxieStudioClient({
                baseUrl: AxieStudio_SERVER_ADDRESS,
                apiKey: AxieStudio_API_KEY
            });

            console.log(`Connecting to AxieStudio server at: ${AxieStudio_SERVER_ADDRESS} `);
            ...
            if (error instanceof Error) {
                if (error.message.includes('fetch')) {
                    console.error('\nMake sure your AxieStudio server is running and accessible at:', AxieStudio_SERVER_ADDRESS);
                }
    ...
    // Run the function
    console.log('Starting AxieStudio Agent...\n');
...
3.  Save and run the script to send the request and test the flow.
    ...
    ```
    Starting AxieStudio Agent...

    Connecting to AxieStudio server at: http://localhost:7860
