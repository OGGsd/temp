...
You can export flows to transfer them between AxieStudio instances, share them with others, or create backups.
...
* **Export with the AxieStudio API**: To export one flow, use the [`/flows/download`](/api-flows#export-flows) endpoint.
...
For more information, see [AxieStudio JSON file contents](#AxieStudio-json-file-contents).
...
If your key is stored in a AxieStudio global variable, **Save with my API keys** exports only the variable name.
:::

When you or another user import the flow to another AxieStudio instance, that instance must have AxieStudio global variables with the same names and valid values in order to run the flow successfully.
If any variables are missing or invalid, those variables must be created or edited after importing the flow.

## Import a flow

You can import AxieStudio JSON files from your local machine in the following ways:

* **Import to projects**: On the **Projects** page, click <Icon name="Upload" aria-hidden="true"/> **Upload a flow**, and then select the AxieStudio JSON file to import.
* **Import anywhere**: Drag and drop AxieStudio JSON files from your file explorer into your AxieStudio window to import a flow from any AxieStudio page.
* **Import with the AxieStudio API**: To import one AxieStudio JSON file, use the [`/flows/upload/`](/api-flows#import-flows) endpoint.
To import a zip archive of AxieStudio JSON files, use the [`/projects/upload`](/api-projects#import-a-project) endpoint.

### Run an imported flow

Once imported, your flow is ready to use.
If the flow contains any global variables, make sure your AxieStudio instance has global variables with the same names and valid values.
For more information, see [Save with my API keys](/concepts-flows-import#save-with-my-api-keys).

## AxieStudio JSON file contents

An exported flow is downloaded to your local machine as a JSON file named `FLOW_NAME.json`.

AxieStudio JSON files contain [nodes](#nodes) and [edges](#edges) that describe components and connections, and [additional metadata](#additional-metadata-and-project-information) that describe the flow.

For an example AxieStudio JSON file, examine the [Basic Prompting.json](https://github.com/AxieStudio-ai/AxieStudio/blob/main/src/backend/base/AxieStudio/initial_setup/starter_projects/Basic%20Prompting.json) file in the AxieStudio repository.
