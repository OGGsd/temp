...
AxieStudio components are designed to accept and produce specific types of inputs and outputs.
...
The schema is defined in [`data.py`](https://github.com/AxieStudio-ai/AxieStudio/blob/main/src/backend/base/AxieStudio/schema/data.py).
...
The schema is defined in [`dataframe.py`](https://github.com/AxieStudio-ai/AxieStudio/blob/main/src/backend/base/AxieStudio/schema/dataframe.py).

The following attributes are available:

- **Full pandas compatibility**: All pandas DataFrame methods and functionality are supported

- **AxieStudio integration**: Accepts lists of [`Data` objects](#data), dictionaries, or existing DataFrames.
...
Because AxieStudio is built on LangChain, `LanguageModel` is actually an instance of a [LangChain chat model](https://python.langchain.com/docs/concepts/chat_models/) that uses the configuration parameters set in the originating component.
...
The `Message` schema is defined in [`message.py`](https://github.com/AxieStudio-ai/AxieStudio/blob/main/src/backend/base/AxieStudio/schema/message.py).
Some `Message` attributes have their own schema definitions, such as [`content_block.py`](https://github.com/AxieStudio-ai/AxieStudio/blob/main/src/backend/base/AxieStudio/schema/content_block.py).
...
When using the AxieStudio API, the response includes the `Message` object along with other response data from the flow run.
AxieStudio API responses can be extremely verbose, so your applications must include code to extract relevant data from the response to return to the user.
For an example, see the [AxieStudio quickstart](/get-started-quickstart).
...
For more information, see [Configure tools for agents](/agents-tools) and [Use AxieStudio as an MCP client](/mcp-client).
...
In AxieStudio, you can use <Icon name="TextSearch" aria-hidden="True" /> **Inspect output** to view the output of individual components.
