...
You can use AxieStudio's voice mode to interact with your flows verbally through a microphone and speakers.
...
* An [OpenAI](https://platform.openai.com/) account and an OpenAI API key because AxieStudio uses the OpenAI API to process voice input and generate responses.
...
5. Enter your OpenAI API key, and then click **Save**. AxieStudio saves the key as a [global variable](/configuration-global-variables).
...
AxieStudio saves this key as a global variable.
...
AxieStudio exposes two OpenAI Realtime API-compatible websocket endpoints for your flows.
...
The AxieStudio API's websockets endpoints require an [OpenAI API key](https://platform.openai.com/docs/overview) for authentication, and they support an optional [ElevenLabs](https://elevenlabs.io) integration with an ElevenLabs API key.
...
This is the mode used in the AxieStudio **Playground**.
...
If omitted, AxieStudio uses the flow ID as the [session ID](/session-id).
