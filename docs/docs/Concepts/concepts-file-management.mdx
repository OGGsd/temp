...
Each AxieStudio server has a file management system where you can store files that you want to use in your flows.

Files uploaded to AxieStudio file management are stored locally to your AxieStudio server, and they are available to all of your flows.

Uploading files to AxieStudio file management keeps your files in a central location, and allows you to reuse files across flows without repeated manual uploads.

## Use the file management UI

You can use the file management UI to upload files from your local machine to your own AxieStudio server.
You can also manage all files that have been uploaded to your AxieStudio server.

1. Navigate to AxieStudio file management:

    * In AxieStudio, on the [**Projects** page](/concepts-flows#projects) page, click **My Files** below the list of projects.
    * From a browser, navigate to your AxieStudio server's `/files` endpoint, such as `http://localhost:7860/files`. Modify the base URL as needed for your AxieStudio server.
    * For programmatic file management, use the [AxieStudio API files endpoints](/api-files). However, the following steps assume you're using the file management UI.
...
## Upload and manage files with the AxieStudio API

With the AxieStudio API, you can upload and manage files in AxieStudio file management, and you can send files to flows programmatically at runtime.
...
To use files in your AxieStudio file management system in a flow, add a component that accepts file input to your flow, such as the **File** component.
...
3. Expand the **File** section, find the **Files** row, and then enable **Expose Input** to allow the parameter to be set at runtime through the AxieStudio API.
...
5. When you run this flow programmatically, your script must upload a file to AxieStudio file management, and then pass the returned `file_path` to the `path` tweak in the `/run` request:
...
AxieStudio supports base64 images in the following formats:
...
You can upload images to the **Playground** chat interface and as runtime input with the AxieStudio API.
...
* When you trigger a flow with the `/api/v1/run/$FLOW_ID` endpoint, you can use the `files` parameter to attach image data as a base64-encoded string:
   ...
   ```bash
   curl -X POST "http://$AxieStudio_SERVER_ADDRESS/api/v1/run/$FLOW_ID" \
...
For videos, see the **Twelve Labs** and **YouTube** [bundles](/components-bundle-components) in the AxieStudio **Components** menu.
