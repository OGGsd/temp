...
:::tip
AxieStudio provides keyboard shortcuts for the workspace.

In the AxieStudio header, click your profile icon, select **Settings**, and then click **Shortcuts** to view the available shortcuts.
:::
...
* **Core**: The categories near the top of the **Components** menu are AxieStudio's core components. They are grouped by purpose, such as **Inputs and Outputs** or **Data**. **Beta** components are newly released and still in development.
...
For information about the programmatic representation of each data type, see [AxieStudio data types](/data-types).
...
For example, the [**Recursive Character Text Splitter** component](https://github.com/AxieStudio-ai/AxieStudio/blob/main/src/backend/base/AxieStudio/components/langchain_utilities/recursive_character.py) is a child of the [`LCTextSplitterComponent`](https://github.com/AxieStudio-ai/AxieStudio/blob/main/src/backend/base/AxieStudio/base/textsplitters/model.py) class.
...
Component versions and states are stored in an internal AxieStudio database. When you add a component to a flow, you create a detached copy of the component based on the information in the AxieStudio database.
These copies are detached from the primary AxieStudio database, and they don't synchronize with any updates that can occur when you upgrade your AxieStudio version.
...
When editing a flow in the workspace, AxieStudio notifies you if a component's workspace version is behind the database version so you can update the component's workspace version:
...
Components are updated to the latest available version, based on the version of AxieStudio you are running.
