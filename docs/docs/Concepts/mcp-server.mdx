---
title:  Use AxieStudio as an MCP server
slug: /mcp-server
---
...
AxieStudio integrates with the [Model Context Protocol (MCP)](https://modelcontextprotocol.io/introduction) as both an MCP server and an MCP client.

This page describes how to use AxieStudio as an MCP server.

For information about using AxieStudio as an MCP client, see [Use AxieStudio as an MCP client](/mcp-client).

As an MCP server, AxieStudio exposes your flows as [tools](https://modelcontextprotocol.io/docs/concepts/tools) that [MCP clients](https://modelcontextprotocol.io/clients) can use to take actions.

## Prerequisites

* A [AxieStudio project](/concepts-flows#projects) with at least one flow.
...
* [ngrok installed](https://ngrok.com/docs/getting-started/#1-install-ngrok) and an [ngrok authtoken](https://dashboard.ngrok.com/get-started/your-authtoken) if you want to [deploy a public AxieStudio server](/deployment-public-server).
...
Each [AxieStudio project](/concepts-flows#projects) has an MCP server that exposes the project's flows as tools that MCP clients can use to generate responses.
...
1. From the AxieStudio dashboard, select the project that contains the flows you want to serve as tools, and then click the **MCP Server** tab.
...
4. Recommended: Edit the **Tool Name** and **Tool Description** to help MCP clients determine which actions your flows provide and when to use those actions:
    ...
    MCP clients like [Cursor](https://www.cursor.com/) treat your AxieStudio project as a single MCP server with all of your enabled flows listed as tools.
    ...
    To provide more context about your flows, make sure to name and describe your flows clearly when configuring your AxieStudio project's MCP server.
    ...
    After connecting your AxieStudio MCP server to Cursor, you can ask Cursor about the resume, such as `What job experience does Emily have?`.
...
{/* The anchor on this section (connect-clients-to-use-the-servers-actions) is currently a link target in the AxieStudio UI. Do not change. */}
## Connect clients to AxieStudio's MCP server {#connect-clients-to-use-the-servers-actions}

The following procedure describes how to connect [Cursor](https://www.cursor.com/) to your AxieStudio project's MCP server to consume your flows as tools.
...
:::important
Auto installation only works if your HTTP client and AxieStudio server are on the same local machine.
...
2. In the AxieStudio dashboard, select the project that contains the flows you want to serve, and then click the **MCP Server** tab.
3. To auto install your current AxieStudio project as an MCP server, click <Icon name="Plus" aria-hidden="True"/> **Add**.
...
3. In the AxieStudio dashboard, select the project that contains the flows you want to serve, and then click the **MCP Server** tab.
...
For example:
    ...
    ```json
    {
      "mcpServers": {
        "PROJECT_NAME": {
          ...
          "args": [
            "mcp-proxy",
            "http://AxieStudio_SERVER_ADDRESS/api/v1/mcp/project/PROJECT_ID/sse"
          ]
        }
      }
    }
    ...
    The **MCP Server** tab automatically includes the correct `PROJECT_NAME`, `AxieStudio_SERVER_ADDRESS`, and `PROJECT_ID` values.
    The default AxieStudio server address is `http://localhost:7860`.

    If you have [deployed a public AxieStudio server](/deployment-public-server), the address is automatically included.

    If your AxieStudio server requires authentication, you must include your AxieStudio API key in the configuration.
...
You must provide a AxieStudio API key in your MCP client configuration if your [AxieStudio server has authentication enabled](/api-keys-and-authentication#start-a-AxieStudio-server-with-authentication-enabled).
...
Click <Icon name="key" aria-hidden="true"/> **Generate API key** to automatically insert a new AxieStudio API key into the code template.
Alternatively, you can replace `YOUR_API_KEY` with an existing AxieStudio API key.
...
{/* The anchor on this section (deploy-your-server-externally) is currently a link target in the AxieStudio UI. Do not change. */}
### Deploy your MCP server externally {#deploy-your-server-externally}

To deploy your MCP server externally with ngrok, see [Deploy a public AxieStudio server](/deployment-public-server).
...
3. In the MCP Inspector UI, enter the connection details for your AxieStudio project's MCP server:

    - **Transport Type**: Select **SSE**.
    - **URL**: Enter the AxieStudio MCP server's `sse` endpoint. For example: `http://localhost:7860/api/v1/mcp/project/d359cbd4-6fa2-4002-9d53-fa05c645319c/sse`

    If you've [configured authentication for your MCP server](#authentication), fill out the following additional fields:
    - **Transport Type**: Select **STDIO**.
    - **Command**: `uvx`
    - **Arguments**: Enter the following list of arguments, separated by spaces. Replace the values for `YOUR_API_KEY`, `AxieStudio_SERVER_ADDRESS`, and `PROJECT_ID` with the values from your AxieStudio MCP server. For example:
    ```bash
    mcp-proxy --headers x-api-key YOUR_API_KEY http://AxieStudio_SERVER_ADDRESS/api/v1/mcp/project/PROJECT_ID/sse
    ```
...
- [Use AxieStudio as an MCP client](/mcp-client)
