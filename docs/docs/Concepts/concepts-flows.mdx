...
AxieStudio flows are fully serializable and can be saved and loaded from the file system where AxieStudio is installed.

:::tip
To try building and running a flow in a few minutes, see the [AxieStudio quickstart](/get-started-quickstart).
:::
...
* **Create a flow from a template**: Select a project, click **New Flow**, and then click the template that you want to use.
    ...
    You can also [contribute templates](/contributing-templates) to the AxieStudio codebase.
...
You can also create a flow with the [AxieStudio API](/api-flows), but the AxieStudio team recommends using the [visual editor](/concepts-overview) until you are familiar with flow creation.
...
When you're ready to use AxieStudio for application development, learn how to [trigger flows with the AxieStudio API](/concepts-publish), explore more advanced configuration options like [custom dependencies](/install-custom-dependencies), and, eventually, [containerize your AxieStudio application](/develop-application).

When you're ready to go to production or deploy a AxieStudio MCP server for access over the public internet, see [AxieStudio deployment overview](/deployment-overview).

#### Flow graphs

When a flow runs, AxieStudio builds a Directed Acyclic Graph (DAG) object from the nodes (components) and edges (connections), and the nodes are sorted to determine the order of execution.
...
The **Projects** page is where you arrive when you launch AxieStudio.
It is where you view and manage flows on a high level.

AxieStudio projects are like folders that you can use to organize related flows.
...
* **Serve flows as MCP tools**: See [Use AxieStudio as an MCP server](/mcp-server).

:::tip
To get back to the **Projects** page after editing a flow, click the project name or AxieStudio icon in the AxieStudio header.
:::
...
- **macOS Desktop**: `/Users/<username>/.AxieStudio/cache`
- **Windows Desktop**: `C:\Users\<username>\AppData\Roaming\com.AxieStudio\cache`
- **OSS macOS/Windows/Linux/WSL (uv pip install)**: `<path_to_venv>/lib/python3.12/site-packages/AxieStudio/cache`
- **OSS macOS/Windows/Linux/WSL (git clone)**: `<path_to_clone>/src/backend/base/AxieStudio/cache`

The overall storage location can be customized with the `AxieStudio_CONFIG_DIR` environment variable, and the flow log storage location can be customized separately with the `AxieStudio_LOG_FILE` environment variable.
...
* [AxieStudio environment variables](/environment-variables)