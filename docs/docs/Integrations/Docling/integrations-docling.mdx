...
AxieStudio integrates with [Docling](https://docling-project.github.io/docling/) through a bundle of components for parsing documents.

## Install Docling dependency

:::important
You must install the Docling dependency to use the Docling components in AxieStudio.
:::

<Tabs>
<TabItem value="oss" label="AxieStudio OSS" default>

Install the Docling extra in AxieStudio OSS with `uv pip install 'AxieStudio[docling]'`.

</TabItem>

<TabItem value="desktop" label="AxieStudio Desktop">

To add a dependency to AxieStudio Desktop, add an entry for Docling to the application's `requirements.txt` file.
For more information, see [Install custom dependencies in AxieStudio Desktop](/install-custom-dependencies#AxieStudio-desktop).
...
:::tip
To learn more about content extraction with Docling, see the video tutorial [Docling + AxieStudio: Document Processing for AI Workflows](https://www.youtube.com/watch?v=5DuS6uRI5OM).
:::
