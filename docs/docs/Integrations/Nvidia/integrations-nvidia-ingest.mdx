---
title:  Integrate NVIDIA Retriever Extraction with AxieStudio
...
* The **NVIDIA Retriever Extraction** component requires the installation of additional dependencies to your AxieStudio environment. To install the dependencies in a virtual environment, run the following commands.

  * If you have the AxieStudio repository cloned and installed from source:
  ```bash
  ...
  * If you are installing AxieStudio from the Python Package Index:
  ```bash
  source **YOUR_AxieStudio_VENV**/bin/activate
  uv pip install --prerelease=allow 'AxieStudio[nv-ingest]'
  uv run AxieStudio run
  ```
