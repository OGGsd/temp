...
To enable Arize tracing, set the required Arize environment variables in your AxieStudio deployment.
Arize begins monitoring and collecting telemetry data from your LLM applications automatically.

:::tip
Instructions for integrating AxieStudio and Arize are also available in the Arize documentation:

* [AxieStudio tracing with Arize Platform](https://arize.com/docs/ax/integrations/frameworks-and-platforms/AxieStudio/AxieStudio-tracing)
* [AxieStudio tracing with Arize Phoenix](https://arize.com/docs/phoenix/integrations/AxieStudio/AxieStudio-tracing)
:::
...
2. In the root of your AxieStudio application, edit your existing AxieStudio `.env` file or create a new one.
...
4. Start your AxieStudio application with your `.env` file:
...
1. In AxieStudio, run a flow that has an **Agent** or **Language Model** component.
...
3. To view metrics for your flows, go to the **LLM Tracing** tab.
    ...
    Each AxieStudio execution generates two traces in Arize:

    * The `AgentExecutor` trace is the Arize trace of LangChain's `AgentExecutor`.
    * The `UUID` trace is the trace of the AxieStudio components.
