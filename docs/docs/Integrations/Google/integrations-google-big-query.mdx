...
AxieStudio integrates with [Google BigQuery](https://cloud.google.com/bigquery) through the **BigQuery** component in the [**Google** bundle](/bundles-google), allowing you to execute SQL queries and retrieve data from your BigQuery datasets.
...
* A [running AxieStudio server](/get-started-installation)
...
This role allows AxieStudio to access BigQuery resources with the service account.
...
Now that you have a service account and a JSON private key, you need to configure the credentials in the AxieStudio **BigQuery** component.

## Configure credentials in the AxieStudio component

With your service account configured and your credentials JSON file created, follow these steps to authenticate the AxieStudio application.

1. Create a new flow in AxieStudio.
...
2. Open the **Playground**, and then submit a valid SQL query.
    ...
    FROM `big-query-AxieStudio-project.the_oscar_award.oscar_winners`
    WHERE winner = TRUE
    LIMIT 10
    ```
