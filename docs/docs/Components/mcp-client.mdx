---
title:  Use AxieStudio as an MCP client
slug: /mcp-client
---
...
This page describes how to use AxieStudio as an MCP client with the [**MCP Tools** component](#use-the-mcp-tools-component) and connected [MCP servers](#manage-connected-mcp-servers).

For information about using AxieStudio as an MCP server, see [Use AxieStudio as an MCP server](/mcp-server).

## Use the MCP tools component

The **MCP Tools** component connects to an MCP server so that a [AxieStudio agent](/agents) can use the server's tools when responding to user queries.

This component has two modes, depending on the type of server you want to access:

* [Connect to a non-AxieStudio MCP server](#mcp-stdio-mode) with a JSON configuration file, server start command, or SSE URL to access tools provided by external, non-AxieStudio MCP servers.
* [Connect to a AxieStudio MCP server](#mcp-sse-mode) to use flows from your [AxieStudio projects](/concepts-flows#projects) as MCP tools.

### Connect to a non-AxieStudio MCP server {#mcp-stdio-mode}
...
2. In the **MCP Server** field, select the server you want to add, or click <Icon name="Plus" aria-hidden="true"/> **Add MCP Server**.
    ...
    * In the **SSE** pane, enter your AxieStudio MCP server's **Name**, **SSE URL**, and any **Headers** or **Environment Variables** the server uses, and then click **Add Server**.
    The default **SSE URL** is `http://localhost:7860/api/v1/mcp/sse`. For more information, see [Use SSE mode](#mcp-sse-mode).

    `uvx` is included with `uv` in the AxieStudio package.
    ...
    For an example of an `npx` MCP server in AxieStudio, see [Connect an Astra DB MCP server to AxieStudio](/mcp-component-astra).
...
3. To use environment variables in your server command, enter each variable in the **Env** fields as you would define them in a script, such as `VARIABLE=value`.
    ...
    :::important
    AxieStudio passes environment variables from the `.env` file to MCP, but it doesn't pass global variables declared in your AxieStudio **Settings**.
    To define an MCP server environment variable as a global variable, add it to AxieStudio's `.env` file at startup.
...
### Connect a AxieStudio MCP server {#mcp-sse-mode}

Every AxieStudio project runs a separate MCP server that exposes the project's flows as MCP tools.
For more information about your projects' MCP servers, including how to manage exposed flows, see [Use AxieStudio as an MCP server](/mcp-server).
...
2. In the **MCP SSE URL** field, modify the default address to point at your AxieStudio server's SSE endpoint. The default value for other AxieStudio installations is `http://localhost:7860/api/v1/mcp/sse`.
...
| sse_url | String | Input parameter. SSE mode only. The SSE URL for a AxieStudio project's MCP server. Default for AxieStudio Desktop: `http://localhost:7868/`. Default for other installations: `http://localhost:7860/api/v1/mcp/sse` |
...
To manage MCP servers connected to your AxieStudio client, go to **Settings**, and then click **MCP Servers**.
