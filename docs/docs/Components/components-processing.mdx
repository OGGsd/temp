...
AxieStudio's **Processing** components process and transform data within a flow.
...
For more information about the webhook endpoint, see [Trigger flows with webhooks](/webhook).
    ...
    ```bash
    curl -X POST "http://$AxieStudio_SERVER_URL/api/v1/webhook/$FLOW_ID" \
...
For example:
    ...
    - Selected AxieStudio Model Name: gpt-4o-mini
...
For more information about the content and structure of `DataFrame` and `Data` objects, see [AxieStudio data types](/data-types).
...
The **Python Interpreter** component can only import packages that are already installed in your AxieStudio environment.
...
Several file formats are supported, and you can store files in [AxieStudio storage](/memory) or the local file system.
...
2. In **File Name**, enter a file name and an optional path.
    ...
    * **Default location**: If you only provide a file name, then the file is stored in `.AxieStudio/data`.

    * **Subdirectory**: To store files in subdirectories, add the path to the **File Name** parameter.
    For example, `subdirectory/my_file` creates `my_file` in `.AxieStudio/data/subdirectory`.
    If a given subdirectory doesn't already exist, AxieStudio automatically creates it.

    * **Absolute or relative path**: To store files elsewhere in your `.AxieStudio` installation or the local file system, provide the absolute or relative path to the desired location.
...
3. In the [component's header menu](/concepts-components#component-menus), click <Icon name="SlidersHorizontal" aria-hidden="true"/> **Controls**, select the desired file format, and then click **Close**.
    ...
    * `Message` can be saved to TXT, JSON (default), or Markdown.
    ...
    AxieStudio doesn't block overwrites if a matching file already exists.
...
4. To test the **Save File** component, click <Icon name="Play" aria-hidden="True" /> **Run component**, and then click <Icon name="TextSearch" aria-hidden="True" /> **Inspect output** to get the filepath where the file was saved.
    ...
    ```text
    DataFrame saved successfully as 'my_file.csv' at /Users/user.name/.AxieStudio/data/my_file.csv
    ```
...
In AxieStudio version 1.5, this component was renamed from **Lambda Filter** to **Smart Function**.
...
For more complex schemas with lists and dictionaries, it might help to refer to the `Data` and `DataFrame` structures and attributes, as described in [AxieStudio data types](/data-types).
...
For more information, see [AxieStudio data types](/data-types).
