...
**Embedding Model** components in AxieStudio generate text embeddings using a specified Large Language Model (LLM).

AxieStudio includes an **Embedding Model** core component that has built-in support for some LLMs.
...
You can enter component API keys directly or use AxieStudio global variables to reference your API keys.
