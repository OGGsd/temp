...
However, AxieStudio's **Agent** and **Language Model** components include built-in chat memory that is enabled by default.
...
For more information, see [Use AxieStudio agents](/agents).
...
| Is Metadata | When loading data using LangChain or AxieStudio, additional attributes may be stored under a metadata object. If the target attribute is stored this way, enable this option. It adjusts the query by generating a filter in the format: `{"metadata.<attribute_name>": "<value>"}` |
...
This component was deprecated in AxieStudio version 1.1.2.
