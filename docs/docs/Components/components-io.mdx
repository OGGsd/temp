...
AxieStudio's **Input and Output** components define where data enters and exits your flow, but they don't have identical functionality.
...
**Chat Input and Output** components are designed to handle conversational interactions in AxieStudio.
...
For information about these data types, see [Use AxieStudio data types](/data-types).
...
When using the AxieStudio API, the API response includes the **Chat Output** `Message` object along with other response data from the flow run.
...
* [AxieStudio quickstart](/get-started-quickstart): Create and run a basic agent flow.
* **Basic Prompting** template: Create an LLM-based chat flow that accepts chat input as well as a prompt with additional instructions for the LLM. Many other AxieStudio templates also use **Chat Input and Output** components.
...
### Send chat input with the AxieStudio API

You can use the AxieStudio API to run a flow by sending input to a **Chat Input** component:
...
When triggering flows with the AxieStudio API, the payload must contain the values for the **Chat Input** component's input parameters, such as `input_value`.
...
For more information, see [Trigger flows with the AxieStudio API](/concepts-publish).
