...
Custom components extend AxieStudio's functionality through Python classes that inherit from `Component`. This enables integration of new features, data manipulation, external services, and specialized tools.

in AxieStudio's node-based environment, each node is a "component" that performs discrete functions. Custom components are Python classes which define:
...
Custom components in AxieStudio are built upon:
...
* `icon`: A decorative icon from axiestudio's icon library, printed next to the name.

    AxieStudio uses [Lucide](https://lucide.dev/icons) for icons. To assign an icon to your component, set the icon attribute to the name of a Lucide icon as a string, such as `icon = "file-text"`. AxieStudio renders icons from the Lucide library automatically.
...
A AxieStudio custom component is more than a class with inputs and outputs. It includes an internal structure with optional lifecycle steps, output generation, front-end interaction, and logic organization.

A basic component:

* Inherits from `axiestudio.custom.Component`.
...
By default, components in AxieStudio that produce multiple outputs only allow one output selection in the visual editor.
...
By default, AxieStudio looks for custom components in the `/components` directory.

If you're creating custom components in a different location using the `AxieStudio_COMPONENTS_PATH` [environment variable](/environment-variables), components must be organized in a specific directory structure to be properly loaded and displayed in the visual editor:

Each category directory **must** contain an `__init__.py` file for AxieStudio to properly recognize and load the components.
...
The category folder name determines where the component appears in the AxieStudio **Components** menu.
For example, to add a component to the **Helpers** category, place it in the `helpers` subfolder:

```
/app/custom_components/          # AxieStudio_COMPONENTS_PATH
...
This folder structure is required for AxieStudio to properly discover and load your custom components. Components placed directly in the base directory aren't loaded.
...
Inputs are defined in a class-level `inputs` list. When AxieStudio loads the component, it uses this list to render component fields and [ports](/concepts-components#component-ports) in the visual editor. Users or other components provide values or connections to fill these inputs.

An input is usually an instance of a class from `axiestudio.io` (such as `StrInput`, `DataInput`, or `MessageTextInput`). The most common constructor parameters are:
...
```python
from axiestudio.io import StrInput, BoolInput, DropdownInput
...
Outputs are defined in a class-level `outputs` list. When AxieStudio renders a component, each output becomes a connector point in the visual editor. When you connect something to an output, AxieStudio automatically calls the corresponding method and passes the returned object to the next component.

An output is usually an instance of `Output` from `axiestudio.io`, with common parameters:
...
AxieStudio currently supports the following input types for **Tool Mode**:
...
in AxieStudio, **typed annotations** allow AxieStudio to visually guide users and maintain flow consistency.

Typed annotations provide:

* **Color-coding**: Outputs like `-> Data` or `-> Message` get distinct colors.
* **Validation**: AxieStudio blocks incompatible connections automatically.
...
In **AxieStudio**, dynamic fields allow inputs to change or appear based on user interactions. You can make an input dynamic by setting `dynamic=True`.
...
When a field with `real_time_refresh=True` is modified, AxieStudio calls the `update_build_config` method, passing the updated field name, value, and the component's configuration to dynamically adjust the visibility or properties of other fields based on user input.
...
in AxieStudio, robust error handling ensures that your components behave predictably, even when unexpected situations occur, such as invalid inputs, external API failures, or internal logic errors.

### Error handling techniques

* **Raise Exceptions**: If a critical error occurs, you can raise standard Python exceptions such as `ValueError`, or specialized exceptions like `ToolException`. AxieStudio will automatically catch these and display appropriate error messages in the visual editor, helping users quickly identify what went wrong.
...
* **Stop specific outputs with `self.stop(...)`**: You can halt individual output paths when certain conditions fail, without affecting the entire component. This is especially useful when working with components that have multiple output branches.
    ...
    if <some condition>:
        self.stop("some_output")  # Tells AxieStudio no data flows
...
## Contribute custom components to AxieStudio

See [How to Contribute](/contributing-components) to contribute your custom component to axiestudio.