...
You can Use AxieStudio's **Data** components to bring data into your flows from various sources like files, API endpoints, and URLs.
For example:
...
If you need to load an unsupported file type, you must use a different component that supports that file type and, potentially, parses it outside AxieStudio, or you must convert it to a supported type before uploading it.
...
| Name | Display Name | Info |
|------|--------------|------|
| path | Files | Input parameter. The path to files to load. Can be local or in [AxieStudio file management](/concepts-file-management). Supports individual files and bundled archives. |
| file_path | Server File Path | Input parameter. A `Data` object with a `file_path` property pointing to a file in [AxieStudio file management](/concepts-file-management) or a `Message` object with a path to the file. Supersedes **Files** (`path`) but supports the same file types. |
...
- **One file**: Produces one of the following depending on the file type. If multiple types are available, you can select the output type by clicking the output field (near the component's output port).
    ...
    - **File Path**: A [`Message`](/data-types#message) containing the path to the file in [AxieStudio file management](/concepts-file-management).
...
```bash
curl -X POST \
  "http://$AxieStudio_SERVER_ADDRESS/api/v1/webhook/$FLOW_ID" \
...
AxieStudio's core components are meant to be generic and support a range of use cases.
