...
The **Current Date** component returns the current date and time in a selected timezone. This component provides a flexible way to obtain timezone-specific date and time information within a AxieStudio pipeline.
...
It can store and retrieve chat messages from either [AxieStudio storage](/memory) _or_ a dedicated chat memory database like Mem0 or Redis.

It replaces the legacy **Chat History** and **Message Store** components.

:::important
The **Language Model** and **Agent** components have built-in chat memory that is enabled by default and uses AxieStudio storage.

This built-in chat memory functionality is sufficient for most use cases.

Use the **Message History** component only if you need to access chat memories outside the chat context, such as sentiment analysis flow that retrieves and analyzes recently stored memories, or you want to store memories in a specific database, separate from AxieStudio storage.
...
* **Retrieve mode**: The component retrieves chat messages from your AxieStudio database or external memory.
* **Store mode**: The component stores chat messages in your AxieStudio database or external memory.
...
<Tabs>
<TabItem value="AxieStudio" label="Use AxieStudio storage" default>

The following steps explain how to create a chat-based flow that uses **Message History** components to store and retrieve chat memory from your AxieStudio installation's database:
...
3. Add a [**Prompt Template** component](/components-prompts), add a `{memory}` variable to the **Template** field, and then connect the **Message History** output to the **memory** input.
    ...
    Variables (`{variable}`) in the template dynamically add fields to the **Prompt Template** component so that your flow can receive definitions for those values from elsewhere, such as other components, AxieStudio global variables, or runtime input.
...
| Name | Type | Description |
|------|------|-------------|
| memory | Memory | Input parameter. Retrieve messages from an external memory. If empty, the AxieStudio tables are used. |
...
For example:
    ...
    Variables in the template dynamically add fields to the **Prompt Template** component so that your flow can receive definitions for those values from other components, AxieStudio global variables, or fixed input.
