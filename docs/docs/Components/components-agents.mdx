...
AxieStudio's **Agent** and **MCP Tools** components are critical for building agent flows.
...
* [AxieStudio quickstart](/get-started-quickstart): Start with the **Simple Agent** template, modify its tools, and then learn how to use an agent flow in an application.

    The **Simple Agent** template creates a basic agent flow with an **Agent** component that can use two other AxieStudio components as tools.
...
* [Use AxieStudio as an MCP client](/mcp-client) and [Use AxieStudio as an MCP server](/mcp-server): Use the **Agent** and **MCP Tools** components to implement the Model Context Protocol (MCP) in your flows.
...
You can connect any AxieStudio component as a tool, including other **Agent** components and MCP servers through the [**MCP Tools** component](#mcp-connection).

For more information about using this component, see [Use AxieStudio agents](/agents).

## MCP Tools component {#mcp-connection}

The **MCP Tools** component connects to a Model Context Protocol (MCP) server and exposes the MCP server's functions as tools for AxieStudio agents to use to respond to input.

In addition to publicly available MCP servers and your own custom-built MCP servers, you can connect AxieStudio MCP servers, which allow your agent to use your AxieStudio flows as tools.
To do this, use the **MCP Tools** component's [SSE mode](/mcp-client#mcp-sse-mode) to connect to your AxieStudio MCP server at the `/api/v1/mcp/sse` endpoint.

For more information about using this component and serving flows as MCP tools, see [Use AxieStudio as an MCP client](/mcp-client) and [Use AxieStudio as an MCP server](/mcp-server).

<details>
<summary>Earlier versions of the MCP Tools component</summary>

* in AxieStudio version 1.5, the **MCP Connection** component was renamed to the **MCP Tools** component.
* in AxieStudio version 1.3, the **MCP Tools (stdio)** and **MCP Tools (SSE)** components were deprecated and replaced by the unified **MCP Connection** component, which was later renamed to **MCP Tools**.
...
Replace these components with the **Agent** component or other AxieStudio components, depending on your use case.
