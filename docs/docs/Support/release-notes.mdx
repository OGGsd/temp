---
title: AxieStudio release notes
slug: /release-notes
---

This page summarizes significant changes to AxieStudio in each release.
For all changes, see the [Changelog](https://github.com/axiestudio/axiestudio/releases/latest).

Due to strict SemVer requirements, AxieStudio Desktop can have different patch versions than the core AxieStudio OSS Python package, but the major and minor versions are aligned.

## Prepare to upgrade

:::important
Whenever possible, the AxieStudio team recommends installing new AxieStudio versions in a new virtual environment or VM before upgrading your primary installation.
...
To avoid the impact of potential breaking changes and test new versions, the AxieStudio team recommends the following upgrade process:

1. Recommended: [Export your projects](/api-projects#export-a-project) to create backups of your flows:

    ```bash
    curl -X GET \
    "$AxieStudio_SERVER_URL/api/v1/projects/download/$PROJECT_ID" \
...
2. Install the new version:
   ...
   * **AxieStudio OSS Python package**: Install the new version in a new virtual environment. For instructions, see [Install and run the AxieStudio OSS Python package](/get-started-installation#install-and-run-the-AxieStudio-oss-python-package).
   * **AxieStudio Docker image**: Run the new image in a separate container.
   * **AxieStudio Desktop**: To upgrade in place, open AxieStudio Desktop, and then click **Upgrade Available** in the AxieStudio header. If you want to isolate the new version, you must install AxieStudio Desktop on a separate physical or virtual machine, and then [import your flows](/concepts-flows-import) to the new installation.
...
For all changes, see the [Changelog](https://github.com/AxieStudio-ai/AxieStudio/releases).

### New features and enhancements

- Authentication changes

    To enhance security and ensure proper authentication for automatic login features, most API endpoints now require authentication with a AxieStudio API key, regardless of the `AUTO_LOGIN` setting.
...
- MCP server one-click installation
    ...
    On your AxieStudio project's **MCP server** page, click **Auto install** to install your AxieStudio MCP server to MCP clients with just one click.
    The option to install with a JSON configuration file is available for macOS, Windows, and WSL.
    For more information, see [Use AxieStudio as an MCP server](/mcp-server).

- MCP server management

    You can now add, remove, and edit your MCP servers in the **MCP Tools** components and through your AxieStudio **Settings** page.
    For more information, see [Use AxieStudio as an MCP client](/mcp-client).
...
- Stability improvements
    ...
    See an issue? [Raise it on GitHub](https://github.com/AxieStudio-ai/AxieStudio/issues).
...
- Added an enhanced **Breaking Changes** feature to help update components without breaking flows after updating AxieStudio.
...
- Introduced MCP server functionality to serve AxieStudio tools to MCP-compatible clients.
...
See the [Changelog](https://github.com/AxieStudio-ai/AxieStudio/releases).