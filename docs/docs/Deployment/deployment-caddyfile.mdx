---
title: Deploy AxieStudio on a remote server
slug: /deployment-caddyfile
---

Learn how to deploy AxieStudio on your own remote server with secure web access.
This guide walks you through setting up AxieStudio on a remote server using [Docker](https://docs.docker.com/) and configuring secure web access with [Caddy](https://caddyserver.com/docs/).
...
## Deploy AxieStudio on your server
...
2. Create a file called `docker-compose.yml`, and then open it in a text editor:
    ...
    ```
    The following example defines the AxieStudio service from the `AxieStudio:latest` image and a Caddy service to expose AxieStudio through a reverse proxy.
    :::tip
    The [host-AxieStudio](https://github.com/datastax/host-AxieStudio) repository offers pre-built copies of this `docker-compose.yml` and `Caddyfile`, if you prefer to fork the repository to your server.
    :::
...
The Caddyfile configures Caddy to listen on port 80, and forward all incoming requests to port 80 to the AxieStudio service at port 7860.
    ```
    :80 {
        reverse_proxy AxieStudio:7860
    }
    ```
6. To deploy your server, run `docker-compose up`.
When the `Welcome to AxieStudio` message appears, AxieStudio is running and accessible internally at `http://0.0.0.0:7860` inside the Docker network.
7. To access your AxieStudio server over the public internet, navigate to your server's public IP address, such as `http://5.161.250.132`.
...
8. Recommended: Enable HTTPS:
    ...
    2. Stop your server.
    3. Modify your Caddyfile to include port `443` so Caddy can forward both HTTP (port 80) and HTTPS (port 443) requests to the AxieStudio service:
...
To package your local flows into a custom Docker image, see [Containerize a AxieStudio application](/develop-application).

For a step-by-step guide to deploying AxieStudio, including deployments to [fly.io](https://fly.io/) and [Flightcontrol.dev](https://www.flightcontrol.dev/), see [How to Host AxieStudio Anywhere](https://www.youtube.com/watch?v=q4qt5hSnte4).