...
AxieStudio uses anonymous telemetry to collect statistics about feature usage and performance.
The AxieStudio team uses this data to identify popular features and areas that need improvement based on actual usage patterns.
This helps prioritize development efforts on the most impactful changes and popular features.

## Privacy

The AxieStudio team respects your privacy, and the team is committed to protecting your data.

AxieStudio telemetry doesn't collect any personal information or sensitive data.
All telemetry data is anonymized and used solely for improving AxieStudio.

## Opt out of telemetry

To opt out of telemetry, set `DO_NOT_TRACK=True` in your [AxieStudio environment variables](/environment-variables) before starting AxieStudio. This disables telemetry data collection.

## Data that AxieStudio collects

AxieStudio telemetry collects data on flow runs, your environment, and component usage.
...
- **Version**: The specific version of AxieStudio used, which helps in tracking feature adoption and compatibility.
...
- **Arch**: Architecture of the system, such as x86 or ARM, which helps prioritize hardware optimization and testing in the AxieStudio codebase.
...
- **BackendOnly**: Boolean indicating whether AxieStudio is running in backend-only mode, useful for understanding deployment configurations.
- **Desktop**: Indicates whether AxieStudio is running in desktop mode (AxieStudio Desktop), helping to understand usage patterns across different deployment types.
