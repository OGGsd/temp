---
title: Contribute to AxieStudio
...
This guide is intended to help you start contributing to AxieStudio.
As an open-source codebase in a rapidly developing field, AxieStudio welcomes contributions, whether it be in the form of a new feature, improved infrastructure, or better documentation.

To contribute code or documentation to AxieStudio, follow the [fork and pull request quickstart](https://docs.github.com/en/get-started/quickstart/contributing-to-projects).

## Install AxieStudio from source

Install AxieStudio from source by forking the repository, and then set up your development environment using Make.
...
### Clone the AxieStudio repository

1. Fork the [AxieStudio GitHub repository](https://github.com/axiestudio/axiestudio).

2. Add the new remote to your local repository on your local machine:
```bash
git remote add FORK_NAME https://github.com/GIT_USERNAME/axiestudio.git
```
Replace the following:
* `FORK_NAME`: A name for your fork of the repository
* `GIT_USERNAME`: Your Git username

### Run AxieStudio from source

You can run AxieStudio from source after cloning the repository, even if you aren't contributing to the codebase.

<details>
<summary>Run from source on macOS/Linux</summary>

In your terminal, navigate to the root of the AxieStudio directory, and then run `make run_cli`.
...
The AxieStudio frontend is served at `http://localhost:7860`.
...
To run AxieStudio from source on Windows, you can use the AxieStudio repository's included scripts, or run the commands in the terminal.

Do one of the following:

* To install and run AxieStudio with the included Windows Batch file, navigate to the `scripts/windows` directory, and then run the `build_and_run.bat` file.

* To run AxieStudio from the Windows Command Line:

    1. Build the AxieStudio frontend:

        ```
        cd src/frontend && npm install && npm run build && npm run start
        ```

    2. Copy the contents of the built `src/frontend/build` directory to `src/backend/base/AxieStudio/frontend`.
...
* To install and run AxieStudio with the included scripts, navigate to the `scripts/windows` directory, and then run the `build_and_run.ps1` file.

* To run AxieStudio from a Powershell terminal:

    1. Run the following commands separately to build the AxieStudio frontend:
...
### Set up your AxieStudio development environment

<Tabs>
<TabItem value="linux-macos" label="Linux or macOS" default>

1. Set up the AxieStudio development environment:
...
Since Windows doesn't include `make`, building and running AxieStudio from source uses `npm` and `uv`.

To set up the AxieStudio development environment, run the frontend and backend in separate terminals:
...
The repo includes a `.vscode/launch.json` file for debugging the backend in VSCode, which is faster than debugging with Docker Compose. To debug AxieStudio with the `launch.json` file in VSCode:
...
2. Fork the [AxieStudio GitHub repository](https://github.com/axiestudio/axiestudio).
...
4. From the root of your local AxieStudio fork, change to the `/docs` directory:
...
6. To edit and create content, work with the `.mdx` files in the `AxieStudio/docs/docs` directory.

    Create new files in `.mdx` format.

    Navigation is defined in `AxieStudio/docs/sidebars.js`.
...
* Pull request titles appear in AxieStudio's release notes, so they should explain what the PR does as explicitly as possible.
* Pull requests should strive to fix one thing **only**, and should contain a good description of what is being fixed.

3. A AxieStudio maintainer will review your pull request and may request changes, so ensure you pay attention to your PRs. Thanks for your contribution!
