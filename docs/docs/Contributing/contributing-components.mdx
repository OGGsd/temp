...
New components are added as objects of the [`Component`](https://github.com/AxieStudio-ai/AxieStudio/blob/main/src/backend/base/AxieStudio/custom/custom_component/component.py) class.

Dependencies are added to the [pyproject.toml](https://github.com/AxieStudio-ai/AxieStudio/blob/main/pyproject.toml#L148) file.

## Contribute an example component to AxieStudio

Anyone can contribute an example component. For example, to create a new data component called **DataFrame processor**, follow these steps to contribute it to AxieStudio.

1. Create a Python file called `dataframe_processor.py`.
2. Write your processor as an object of the [`Component`](https://github.com/AxieStudio-ai/AxieStudio/blob/main/src/backend/base/AxieStudio/custom/custom_component/component.py) class. You'll create a new class, `DataFrameProcessor`, that will inherit from `Component` and override the base class's methods.
...
class DataFrameProcessor(Component):
    ...
    documentation: str = "https://docs.AxieStudio.org/components-dataframe-processor"
...
4. Define the component's interface by specifying its inputs, outputs, and the method that will process them. The method name must match the `method` field in your outputs list, as this is how AxieStudio knows which method to call to generate each output.
...
5. Save the `dataframe_processor.py` to the `src > backend > base > AxieStudio > components` directory.
This example adds a data component, so add it to the `/data` directory.

6. Add the component dependency to `src > backend > base > AxieStudio > components > data > __init__.py` as `from .DataFrameProcessor import DataFrameProcessor`.
You can view the [/data/__init__.py](https://github.com/AxieStudio-ai/AxieStudio/blob/dev/src/backend/base/AxieStudio/components/data/__init__.py) in the AxieStudio repository.

7. Add any new dependencies to the [pyproject.toml](https://github.com/AxieStudio-ai/AxieStudio/blob/main/pyproject.toml#L20) file.

8. Submit documentation for your component. For this example component, you would submit documentation to the [Data components page](https://github.com/AxieStudio-ai/AxieStudio/blob/main/docs/docs/Components/components-data.mdx).

9. Submit your changes as a pull request. The AxieStudio team will review, suggest changes, and add your component to AxieStudio.
