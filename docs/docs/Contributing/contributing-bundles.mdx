...
Follow these steps to add components to the **Bundles** section of the **Components** menu in the AxieStudio visual editor.

Example adds a new bundle named `DarthVader`.

## Add the bundle to the backend folder

1. Navigate to the backend directory in the AxieStudio repository and create a new folder for your bundle.
The path for your new component is `src > backend > base > AxieStudio > components > darth_vader`.
You can view the [components folder](https://github.com/AxieStudio-ai/AxieStudio/tree/main/src/backend/base/AxieStudio/components) in the AxieStudio repository.
...
For an example of adding multiple components in a bundle, see the [Notion](https://github.com/AxieStudio-ai/AxieStudio/tree/main/src/backend/base/AxieStudio/components/Notion) bundle.


## Add the bundle to the frontend folder

1. Navigate to the frontend directory in the AxieStudio repository to add your bundle's icon.
The path for your new component icon is `src > frontend > src > icons > DarthVader`
You can view the [icons folder](https://github.com/AxieStudio-ai/AxieStudio/tree/main/src/frontend/src/icons) in the AxieStudio repository.
...
You can view the [lazyIconImports.ts](https://github.com/AxieStudio-ai/AxieStudio/blob/main/src/frontend/src/icons/lazyIconImports.ts) in the AxieStudio repository.
...
You can view the [SIDEBAR_BUNDLES array](https://github.com/AxieStudio-ai/AxieStudio/blob/main/src/frontend/src/utils/styleUtils.ts#L231) in the AxieStudio repository.\
The `name` must point to the folder you created within the `src > backend > base > AxieStudio > components` directory.
For example:
```typescript
...
1. To rebuild the backend and frontend, run `make install_frontend && make build_frontend && make install_backend && uv run AxieStudio run --port 7860`.
