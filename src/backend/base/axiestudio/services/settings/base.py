...
# in the axiestudio directory
    ...
    Controlled by AXIESTUDIO_USE_NOOP_DATABASE env variable."""
    ...
    """If set to True, Axie Studio will expose Prometheus metrics."""
    prometheus_port: int = 9090
    """The port on which Axie Studio will expose Prometheus metrics. 9090 is the default port."""
    ...
    with the same name in case Axie Studio fails to retrieve the variable value."""
    ...
    log_file: str | None = "logs/axiestudio.log"
    ...
    """The interval in ms at which Axie Studio will auto save flows."""
    ...
    """If set to False, Axie Studio will not enable the MCP server."""
    mcp_server_enable_progress_notifications: bool = False
    """If set to False, Axie Studio will not send progress notifications in the MCP server."""
    ...
    def set_dev(cls, value):
        from axiestudio.settings import set_dev
    ...
    @classmethod
    def set_axiestudio_dir(cls, value):
        if not value:
    ...
    def set_components_path(cls, value):
        ...
        If the `AXIESTUDIO_COMPONENTS_PATH` environment variable is set and points to an existing path, it is
        appended to the provided list if not already present. If the input list is empty or missing, it is
        set to an empty list.
        """
        if os.getenv("AXIESTUDIO_COMPONENTS_PATH"):
            logger.debug("Adding AXIESTUDIO_COMPONENTS_PATH to components_path")
            axiestudio_component_path = os.getenv("AXIESTUDIO_COMPONENTS_PATH")
            if Path(axiestudio_component_path).exists() and axiestudio_component_path not in value:
                if isinstance(axiestudio_component_path, list):
                    for path in axiestudio_component_path:
                        if path not in value:
                            value.append(path)
                    logger.debug(f"Extending {axiestudio_component_path} to components_path")
                elif axiestudio_component_path not in value:
                    value.append(axiestudio_component_path)
                    logger.debug(f"Appending {axiestudio_component_path} to components_path")
    ...
    model_config = SettingsConfigDict(validate_assignment=True, extra="ignore", env_prefix="AXIESTUDIO_")
